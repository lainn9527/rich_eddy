...
from src.strategy.intra_strategy import IntraStrategy
...
def main(arguments: argparse.Namespace, config):
    ...
    cash = 10000000000
    data_store = DataStore()
    if arguments.path != None and not arguments.path.startswith("future_result"):
        result_path = Path("future_result") / arguments.path
    else:
        result_path = arguments.path
    strategy = IntraStrategy(platform, data_store, cash=cash, config=config)
    platform.run(strategy, start_date, end_date, result_path, full_record=True)

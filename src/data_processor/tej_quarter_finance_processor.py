...
class TejQuarterFinanceProcessor(TejDataProcessor):
    def extract_code_and_date_from_line(line: List[str]):
        ...
        return code, release_datetime, line[2:]
    ...
    #         lines = TejQuarterFinanceProcessor.remove_null_token(fp.readlines())
    #         lines = list(csv.reader(lines))

    #         raw_column_names = TejQuarterFinanceProcessor.get_column_names("finance_report")
    #         for line in lines[1:]:
    ...
    #     with open(dest_data_dir / "full.csv", "w") as fp:

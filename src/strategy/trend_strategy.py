...
class TrendStrategy(Strategy):
    ...
    def prepare_data(self, start_date: datetime, end_date: datetime):
        ...
        self.foreign_total_holdings_ratio_sma_ = self.data_store.get_technical_indicator(
            TechnicalIndicator.SMA, self.foreign_total_holdings_ratio_, self.config["chip_strategy"]["foreign_total_holdings_ratio_sma_period"])
        self.local_self_holdings_ratio_sma_ = self.data_store.get_technical_indicator(
            TechnicalIndicator.SMA, self.local_self_holdings_ratio_, self.config["chip_strategy"]["local_self_holdings_ratio_sma_period"])
        self.local_investor_holdings_ratio_sma_ = self.data_store.get_technical_indicator(
            TechnicalIndicator.SMA, self.local_investor_holdings_ratio_, self.config["chip_strategy"]["local_investor_holdings_ratio_sma_period"])
    ...
    def get_signal(self):
        ...
        for code_idx in range(local_max_array.shape[1]):
            ...
            for i, local_max_idx in enumerate(local_max_idx_list):
                ...
                # if volume[breakthrough_point_idx] < volume_long_sma[breakthrough_point_idx]:
                #     volume_filtered += 1
                #     filtered_reason[local_max_idx, code_idx] = 10
                #     continue
        ...
        print(f"volume_filtered: {(100 * volume_filtered/total_signal):.2f}")
